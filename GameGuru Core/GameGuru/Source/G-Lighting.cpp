...
void lighting_loop(void)
{
	...
	if (t.playerlight.flashlightcontrol_f > 0.0f)
	{
		...
		if (uiFlashLight) 
		{
			...
			MoveCamera(-5.0);
			WickedCall_UpdateLight(uiFlashLight, fCamX, fCamY, fCamZ, fCamAX, fCamAY, fCamAZ, 700.0f, 45.0f, 64, 64, 64, true);
		}
	}
	else 
	{
		...
		if (uiFlashLight)
		{
			...
			WickedCall_UpdateLight(uiFlashLight, fCamX, fCamY, fCamZ, fCamAX, fCamAY, fCamAZ, 700.0f, 45.0f, 0, 0, 0, true);
		}
		#endif // WICKEDENGINE
		if (bFlashLightIsOn) 
		{
			//LB: fix issue of stutter (env probe recalc hit), keep the light, just make it dark :)
			//WickedCall_DeleteLight(uiFlashLight);
