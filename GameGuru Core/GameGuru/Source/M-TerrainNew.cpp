...
void imgui_Customize_Terrain_v3(int mode)
{
	...
	if (ImGui::StyleCollapsingHeader("Palette", wflags))
	{
		...
		if (t.visuals.sTerrainTextures[0] != "terraintextures\\mat1\\Color.dds" || t.visuals.sTerrainTexturesName[0] != "Rainforest Overgrowth" )
		{
			//Add a few default textures.
			//PE: We always need a default texture in [0].
...
void imgui_Customize_Sky_V2(int mode)
{
	...
	if (ImGui::StyleCollapsingHeader("Customize Sky", wflags))
	{
		...
		void* lpTexture2 = GetImagePointer(TOOL_PENCIL);
...
//						fSnapShotModeCameraZ = newTargetCamera.y;
		...
		if (bPopModalOpenProcedural)
		{
			...
			//rDigAHole.left = ImGui::GetWindowPos().x + 2.0;
			//rDigAHole.top = ImGui::GetWindowPos().y + 44.0; //Header.
			//rDigAHole.right = ImGui::GetWindowPos().x + fPreviewImgSize;
			//rDigAHole.bottom = ImGui::GetWindowPos().y + ImGui::GetWindowSize().y - 24.0;

			rDigAHole.left = 2.0;
			rDigAHole.top = 67.0;
			rDigAHole.right = fPreviewImgSize;
			rDigAHole.bottom = ImGui::GetWindowSize().y - 24.0;
			...
			#endif
			{
				...
				if (!bTriggerTerrainSaveAsWindow && !bLastSmallVideoPlayerMaximized)
				{
					...
					RECT rDigAHole_IMGUI_POS;
					rDigAHole_IMGUI_POS.left = ImGui::GetWindowPos().x + 2.0;
					rDigAHole_IMGUI_POS.top = ImGui::GetWindowPos().y + 44.0; //Header.
					rDigAHole_IMGUI_POS.right = ImGui::GetWindowPos().x + fPreviewImgSize;
					rDigAHole_IMGUI_POS.bottom = ImGui::GetWindowPos().y + ImGui::GetWindowSize().y - 24.0;

					ImVec2 min = { (float)rDigAHole_IMGUI_POS.left, (float)rDigAHole_IMGUI_POS.top };
					ImVec2 max = { (float)rDigAHole_IMGUI_POS.right, (float)rDigAHole_IMGUI_POS.bottom };
...
//			ImGui::Text("ang: %.2f,%.2f,%.2f", fSnapShotModeCameraAngX, fSnapShotModeCameraAngY, fSnapShotModeCameraAngZ);
		...
		if (!bProceduralLevel)
		{
			...
			//rDigAHole.left = ImGui::GetWindowPos().x + 2.0;
			//rDigAHole.top = ImGui::GetWindowPos().y + 80.0; //Header.
			//rDigAHole.right = ImGui::GetWindowPos().x + fPreviewImgSize; //Header.
			//rDigAHole.bottom = ImGui::GetWindowPos().y + ImGui::GetWindowSize().y; //Header.
			#endif
