...
bool cSpecialEffect::CorrectFXFile ( LPSTR pFile, LPSTR pModifiedFile )
{
	...
	if(hreadfile!=INVALID_HANDLE_VALUE)
	{
		...
		// write new temp file
		DWORD actualnewdatasize = (DWORD)(pWritePtr-newbuffer);
...
DARKSDK_DLL bool LoadDBODataBlock ( LPSTR pFilename, DWORD* pdwBlockSize, void** ppDBOBlock )
{
	...
	char pExtension[256];
	strcpy(pExtension, "");
	for ( int n=(int)strlen(pFilename); n>0; n-- )
	{
		if ( pFilename[n]=='.' )
		{
...
DARKSDK_DLL bool LoadDBO ( LPSTR pPassedInFilename, sObject** ppObject )
{
	...
	*ppObject = NULL;
	...
	char pExtension[256];
	strcpy(pExtension, "");
	for ( int n=(int)strlen(pFilename); n>0; n-- ) { if ( pFilename[n]=='.' ) { strcpy(pExtension, pFilename+n+1); break; } }
	...
	if ( _stricmp ( pExtension, "DBO" ) == NULL )
	{
		...
		for (int n = 0; n < g_object_outputv.size(); n++)
		{
			int iSearchStrLen = (int)strlen(pFilename);
