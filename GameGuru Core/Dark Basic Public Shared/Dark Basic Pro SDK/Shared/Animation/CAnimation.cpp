...
#include "../../../../WICKEDREPO/WickedEngine/wiGraphics.h"
#include "../../../../WICKEDREPO/WickedEngine/wiRenderer.h"
...
struct
{
	...
	wiGraphics::Texture			pTexture;
	LPGGSHADERRESOURCEVIEW		pObsolete;//pTextureRef;
	float						ClipU;
	float						ClipV;
	RECT						StreamRect;
	int							x1;
...
BOOL CoreLoadAnimation( int AnimIndex, char* Filename, int precacheframes)
{
	...
	if ( Anim[AnimIndex].pMediaClip )
	{
		...
		wiGraphics::TextureDesc texDesc = {};
		texDesc.ArraySize = 1;
		...
		texDesc.type = TextureDesc::TEXTURE_2D;
		texDesc.Usage = USAGE_DEFAULT;
		device->CreateTexture( &texDesc, nullptr, &Anim[AnimIndex].pTexture );
		device->SetName( &Anim[AnimIndex].pTexture, Filename );
		
		Anim[AnimIndex].ClipU = 1.0f;//fWidth / tTexWidthPOT;
		Anim[AnimIndex].ClipV = 1.0f;//fHeight / tTexHeightPOT;
...
// texture views are now buried in WickedEngine
/*
void* GetAnimPointerView(int AnimIndex)
{
	if (Anim[AnimIndex].pMediaClip ) 
	{
...
DARKSDK void UpdateAllAnimation()
{
	// do nothing, texture can only be updated with a valid CommandList, so do it during rendering update
}

// called from WickedEngine wiRenderer::UpdateRenderData()
extern "C" void UpdateAnimationTexture( CommandList cmd )
{
	...
	for(int AnimIndex=0; AnimIndex<ANIMATIONMAX; AnimIndex++)
	{
		if(Anim[AnimIndex].pMediaClip)
		{
			...
			if ( animation[AnimIndex].playing == true )
			{
				...
				#endif
				if (frame != NULL && Anim[AnimIndex].pTexture.IsValid() )
				{
					...
					uint32_t* rgbData = new uint32_t[ dwVideoWidth * dwVideoHeight ];
					uint32_t* pRGB = rgbData;
					for ( int y = 0; y < iVideoHeight; y++ )
					{
						...
						for ( int x = 0; x < iVideoWidth/2; x++ )
						{
							...
							*pRGB = 0xff000000 | (iBlue << 16) | (iGreen << 8) | iRed;
							pRGB++;
							...
							*pRGB = 0xff000000 | (iBlue2 << 16) | (iGreen2 << 8) | iRed2;
							pRGB++;
					...
					uint32_t stride = dwVideoWidth * sizeof(uint32_t);
					device->UpdateTexture( &Anim[AnimIndex].pTexture, 0, 0, 0, rgbData, stride, cmd );
