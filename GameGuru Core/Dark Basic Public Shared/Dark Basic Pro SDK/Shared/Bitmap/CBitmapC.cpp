...
namespace
{
    ...
    bool RemoveBitmap( int iID )
    {
        ...
        {
            if ( pBitmap->second->lpSurface ) delete (wiGraphics::Texture*) pBitmap->second->lpSurface;
			pBitmap->second->lpSurface = 0;
            if ( pBitmap->second->lpDepth ) delete (wiGraphics::Texture*) pBitmap->second->lpDepth;
			pBitmap->second->lpDepth = 0;
...
DARKSDK void BitmapDestructor ( void )
{
    ...
    for (BitmapPtr pCheck = m_List.begin(); pCheck != m_List.end(); ++pCheck)
    {
		...
		if ( ptr->lpSurface ) delete (wiGraphics::Texture*) ptr->lpSurface;
		ptr->lpSurface = 0;
        if ( ptr->lpDepth ) delete (wiGraphics::Texture*) ptr->lpDepth;
		ptr->lpDepth = 0;
...
DARKSDK LPGGSURFACE MakeFormat ( int iID, int iWidth, int iHeight, GGFORMAT format )
{
	...
	if ( g_bOffscreenBitmap )
	{
		...
		desc.ArraySize = 1;
		desc.BindFlags = 0;
		desc.CPUAccessFlags = wiGraphics::CPU_ACCESS_READ | wiGraphics::CPU_ACCESS_WRITE;
		desc.Width = iWidth;
		desc.Height = iHeight;
		desc.MipLevels = 1;
		desc.ArraySize = 1;
		desc.MiscFlags = 0;
		desc.Usage = wiGraphics::USAGE_STAGING;
		desc.layout = wiGraphics::IMAGE_LAYOUT_UNDEFINED;
	...
	else
	{
		...
		wiGraphics::TextureDesc desc;
		desc.ArraySize = 1;
		desc.BindFlags = wiGraphics::BIND_RENDER_TARGET | wiGraphics::BIND_SHADER_RESOURCE;
		...
		desc.layout = wiGraphics::IMAGE_LAYOUT_RENDERTARGET;
		desc.Format = wiGraphics::FORMAT_B8G8R8A8_UNORM;
		desc.type = wiGraphics::TextureDesc::TEXTURE_2D;
		...
		if ( test->lpDepth==NULL )
		{
			wiGraphics::TextureDesc desc;
			desc.ArraySize = 1;
			desc.BindFlags = wiGraphics::BIND_DEPTH_STENCIL;
			desc.CPUAccessFlags = 0;
			desc.Width = iWidth;
			desc.Height = iHeight;
			desc.MipLevels = 1;
			...
			desc.MiscFlags = 0;
			desc.Usage = wiGraphics::USAGE_DEFAULT;
			desc.layout = wiGraphics::IMAGE_LAYOUT_DEPTHSTENCIL;
			desc.Format = wiGraphics::FORMAT_D24_UNORM_S8_UINT;
			desc.type = wiGraphics::TextureDesc::TEXTURE_2D;
		
			wiGraphics::Texture* pTexture = new wiGraphics::Texture();
			test->lpDepth = pTexture;
			bool result = wiRenderer::GetDevice()->CreateTexture( &desc, nullptr, pTexture );
		}
	}
...
DARKSDK void SetCurrentBitmap ( int iID )
{
	...
	else
	{
		if ( iID > 0 )
		{
			...
			SetRenderAndDepthTarget ( m_camptr->lpSurface, m_camptr->lpDepth );
			g_pGlob->pCurrentBitmapSurfaceView = m_camptr->lpSurface;
			g_pGlob->pCurrentBitmapDepthView = m_camptr->lpDepth;
