...
)
{
    ...
    GGVECTOR4 Result;
	Result.x = (float) ( (VV1.x & ~Control.x) | (VV2.x & Control.x) );
    Result.y = (float) ( (VV1.y & ~Control.y) | (VV2.y & Control.y) );
    Result.z = (float) ( (VV1.z & ~Control.z) | (VV2.z & Control.z) );
    Result.w = (float) ( (VV1.w & ~Control.w) | (VV2.w & Control.w) );
    return Result;
}
