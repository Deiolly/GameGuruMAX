...
#include "../../../../../WICKEDREPO/WickedEngine/wiGraphicsDevice.h"
...
wiGraphics::GraphicsDevice *xrDevice = 0;
...
struct AppSwapChain
{
	...
	uint32_t width;
	uint32_t height;
	wiGraphics::Texture** textures;
	wiGraphics::RenderPass** renderPasses;
};
...
wiGraphics::FORMAT xrSwapchainFormatCommon = wiGraphics::FORMAT_UNKNOWN;
...
int OpenXRInit( wiGraphics::GraphicsDevice* device )
{
	...
	if ( device->GetType() == GRAPHICS_DEVICE_TYPE_DX11 ) extensions[0] = XR_KHR_D3D11_ENABLE_EXTENSION_NAME;
	else if ( device->GetType() == GRAPHICS_DEVICE_TYPE_DX12 ) extensions[0] = XR_KHR_D3D12_ENABLE_EXTENSION_NAME;
	else
	{
		AppError( "Unknown graphics pipeline in use" );
		return -1;
	}
	...
	if ( device->GetType() == GRAPHICS_DEVICE_TYPE_DX11 )
	{
	...
	else if ( device->GetType() == GRAPHICS_DEVICE_TYPE_DX12 ) 
	{
		PFN_xrGetD3D12GraphicsRequirementsKHR pfnGetD3D12GraphicsRequirementsKHR = nullptr;
        xrRes = xrGetInstanceProcAddr( xrInstance, "xrGetD3D12GraphicsRequirementsKHR", (PFN_xrVoidFunction*) &pfnGetD3D12GraphicsRequirementsKHR );
		if ( xrRes < 0 ) { AppError( "xrGetInstanceProcAddr failed" ); }
		if ( pfnGetD3D12GraphicsRequirementsKHR == nullptr ) { AppError( "pfnGetD3D12GraphicsRequirementsKHR is null" ); }

        // Create the D3D12 device for the adapter associated with the system.
        XrGraphicsRequirementsD3D12KHR graphicsRequirements = { XR_TYPE_GRAPHICS_REQUIREMENTS_D3D12_KHR };
        xrRes = pfnGetD3D12GraphicsRequirementsKHR( xrInstance, xrSystemId, &graphicsRequirements );
		if ( xrRes < 0 ) { AppError( "pfnGetD3D12GraphicsRequirementsKHR failed" ); }
	}
	else
	{
		AppError( "Unknown graphics pipeline in use" );
...
void OpenXRStartSession()
{
	...
	if ( xrSystemId == 0 )
	{
		...
		if ( check_count >= 100 )
		{
			...
			if ( xrDevice->GetType() == GRAPHICS_DEVICE_TYPE_DX11 )
			{
				// get graphics requirements
				PFN_xrGetD3D11GraphicsRequirementsKHR pfnGetD3D11GraphicsRequirementsKHR = nullptr;
				xrRes = xrGetInstanceProcAddr( xrInstance, "xrGetD3D11GraphicsRequirementsKHR", (PFN_xrVoidFunction*) &pfnGetD3D11GraphicsRequirementsKHR );
			...
			else if ( xrDevice->GetType() == GRAPHICS_DEVICE_TYPE_DX12 ) 
			{
				PFN_xrGetD3D12GraphicsRequirementsKHR pfnGetD3D12GraphicsRequirementsKHR = nullptr;
				xrRes = xrGetInstanceProcAddr( xrInstance, "xrGetD3D12GraphicsRequirementsKHR", (PFN_xrVoidFunction*) &pfnGetD3D12GraphicsRequirementsKHR );
				if ( xrRes < 0 ) { AppError( "xrGetInstanceProcAddr failed" ); }
	...
	const uint32_t  numSupportedFormats = 4;
	int64_t supportedFormats[ numSupportedFormats ] = { 0 };

	if ( xrDevice->GetType() == GRAPHICS_DEVICE_TYPE_DX11 )
	{
		XrGraphicsBindingD3D11KHR xrGraphicsBinding = { XR_TYPE_GRAPHICS_BINDING_D3D11_KHR };
		xrGraphicsBinding.device = (ID3D11Device*) xrDevice->GetInternalDevice();
		...
		supportedFormats[ 0 ] = DXGI_FORMAT_B8G8R8A8_UNORM_SRGB;
		supportedFormats[ 1 ] = DXGI_FORMAT_R8G8B8A8_UNORM_SRGB;
		supportedFormats[ 2 ] = DXGI_FORMAT_B8G8R8A8_UNORM;
		supportedFormats[ 3 ] = DXGI_FORMAT_R8G8B8A8_UNORM;
	}
	else if ( xrDevice->GetType() == GRAPHICS_DEVICE_TYPE_DX12 ) 
	{
		XrGraphicsBindingD3D12KHR xrGraphicsBinding = { XR_TYPE_GRAPHICS_BINDING_D3D12_KHR };
		xrGraphicsBinding.device = (ID3D12Device*) xrDevice->GetInternalDevice();
		xrGraphicsBinding.queue = (ID3D12CommandQueue*) xrDevice->GetInternalQueue();
	...
	XrResult xrRes = xrCreateReferenceSpace( xrSession, &referenceSpaceCreateInfo, &xrAppSpace);
	...
	{
		for( uint32_t j = 0; j < numSupportedFormats; j++ )
		{
			if ( swapchainFormats[i] == supportedFormats[j] )
			{
				xrSwapchainFormat = swapchainFormats[i];
				break;
			}
		}
		if ( xrSwapchainFormat != 0 ) break;
	}
	...
	if ( xrDevice->GetType() == GRAPHICS_DEVICE_TYPE_DX11 )
	{
		switch ( xrSwapchainFormat )
		{
			case DXGI_FORMAT_B8G8R8A8_UNORM_SRGB: 
			case DXGI_FORMAT_B8G8R8A8_UNORM:
			{
				// both sRGB and non-sRGB render targets must use the non-sRGB format for the image view since the WickedEngine shader does the gamma correction itself
				xrSwapchainFormatCommon = wiGraphics::FORMAT_B8G8R8A8_UNORM;
			} break;
			...
			{
				// both sRGB and non-sRGB render targets must use the non-sRGB format for the image view since the WickedEngine shader does the gamma correction itself
				xrSwapchainFormatCommon = wiGraphics::FORMAT_R8G8B8A8_UNORM;
			} break;

			default:
			{
				AppError( "Failed to find common swapchain format for DX11 specific format" );
			}
		}
	}
	else if ( xrDevice->GetType() == GRAPHICS_DEVICE_TYPE_DX11 )
	{
		switch ( xrSwapchainFormat )
		{
			case DXGI_FORMAT_B8G8R8A8_UNORM_SRGB: 
			...
			default:
			{
				AppError( "Failed to find common swapchain format for DX12 specific format" );
			}
		}
	}
	else
    ...
    for (uint32_t i = 0; i < viewCount; i++) 
	{
		...
		if ( xrRes < 0 || appSwapChains[i].numImages == 0 ) { AppError( "xrEnumerateSwapchainImages failed" ); }

		appSwapChains[i].textures = new wiGraphics::Texture*[ appSwapChains[i].numImages ];
		appSwapChains[i].renderPasses = new wiGraphics::RenderPass*[ appSwapChains[i].numImages ];
		for( uint32_t j = 0; j < appSwapChains[i].numImages; j++ ) 
		{
			appSwapChains[i].textures[j] = 0;
			appSwapChains[i].renderPasses[j] = 0;
		}
		
		if ( xrDevice->GetType() == GRAPHICS_DEVICE_TYPE_DX11 )
		{
			XrSwapchainImageD3D11KHR* images = new XrSwapchainImageD3D11KHR[ appSwapChains[i].numImages ];
			for( uint32_t j = 0; j < appSwapChains[i].numImages; j++ ) 
			{
				images[j] = { XR_TYPE_SWAPCHAIN_IMAGE_D3D11_KHR };
			}

			appSwapChains[i].images = (XrSwapchainImageBaseHeader*) images;
		}
		else if ( xrDevice->GetType() == GRAPHICS_DEVICE_TYPE_DX12 )
		{
			XrSwapchainImageD3D12KHR* images = new XrSwapchainImageD3D12KHR[ appSwapChains[i].numImages ];
			for( uint32_t j = 0; j < appSwapChains[i].numImages; j++ ) 
			{
				images[j] = { XR_TYPE_SWAPCHAIN_IMAGE_D3D12_KHR };
			}
        ...
        xrRes = xrEnumerateSwapchainImages( appSwapChains[i].swapchain, appSwapChains[i].numImages, &appSwapChains[i].numImages, appSwapChains[i].images );
...
{
	if ( xrRenderState != OPENXR_RENDER_STATE_END_FRAME ) { AppError( "OpenXR functions must be called in the correct order" ); return -1; }
	if ( side < 0 || side > 1 ) { AppError( "Invalid render side" ); return -1; }
	...
	if ( xrRes < 0 ) { AppError( "xrAcquireSwapchainImage failed" ); return -1; }

	XrSwapchainImageWaitInfo waitInfo = { XR_TYPE_SWAPCHAIN_IMAGE_WAIT_INFO };
	waitInfo.timeout = XR_INFINITE_DURATION;
	xrRes = xrWaitSwapchainImage( appSwapChains[side].swapchain, &waitInfo );
	if ( xrRes < 0 ) { AppError( "xrWaitSwapchainImage failed" ); return -1; }
	...
	if ( appSwapChains[side].textures[imageIndex] == 0 )
	{
		void* pInternalTexture = 0;
		if ( xrDevice->GetType() == GRAPHICS_DEVICE_TYPE_DX11 )
		{
			XrSwapchainImageD3D11KHR* pRef = &(((XrSwapchainImageD3D11KHR*) appSwapChains[side].images)[imageIndex]);
			pInternalTexture = pRef->texture;
			assert( pInternalTexture != 0 );
		}
		else if ( xrDevice->GetType() == GRAPHICS_DEVICE_TYPE_DX12 )
		{
			XrSwapchainImageD3D12KHR* pRef = &(((XrSwapchainImageD3D12KHR*) appSwapChains[side].images)[imageIndex]);
			pInternalTexture = pRef->texture;
			assert( pInternalTexture != 0 );
		}
		else
		...
		desc.BindFlags = wiGraphics::BIND_RENDER_TARGET;
		desc.SampleCount = 1;
		desc.MipLevels = 1;
		desc.ArraySize = 1;
		desc.Format = xrSwapchainFormatCommon;
		desc.Usage = wiGraphics::USAGE_DEFAULT;
		desc.Width = appSwapChains[side].width;
		desc.Height = appSwapChains[side].height;

		appSwapChains[side].textures[imageIndex] = new wiGraphics::Texture();
		bool result = xrDevice->CreateTextureExternal( &desc, pInternalTexture, appSwapChains[side].textures[imageIndex] );
		if ( !result ) { AppError( "CreateTextureExternal failed" ); return -1; }

		appSwapChains[side].renderPasses[imageIndex] = new wiGraphics::RenderPass();
		wiGraphics::RenderPassDesc renderPassDesc = {};
		renderPassDesc.attachments.push_back( { wiGraphics::RenderPassAttachment::RENDERTARGET, wiGraphics::RenderPassAttachment::LOADOP_DONTCARE, appSwapChains[side].textures[imageIndex], 0 } );
		xrDevice->CreateRenderPass( &renderPassDesc, appSwapChains[side].renderPasses[imageIndex] );
	}

	xrRenderState = OPENXR_RENDER_STATE_RENDERING;

	xrDevice->RenderPassBegin( appSwapChains[side].renderPasses[imageIndex], cmd );
