...
void WickedCall_SetMeshDisableDepth(sMesh* pMesh)
{
	if (pMesh)
	{
		wiScene::MeshComponent* mesh = wiScene::GetScene().meshes.GetComponent(pMesh->wickedmeshindex);
...
void WickedCall_SetObjectDisableDepth(sObject* pObject)
{
	for (int iM = 0; iM < pObject->iMeshCount; iM++)
	{
		if (pObject->ppMeshList[iM])
		{
			...
			//if (bWickedMaterialActive)
			//{
			WickedCall_SetMeshDisableDepth(pObject->ppMeshList[iM]);
			//}
		}
	}
}
	...
	if (pMesh)
	{
		...
		if (mesh)
		{
			...
			if (pObjectMaterial && pMesh->pTextures)
			{
				...
				if (pMeshTexture)
				{
					void* pTexturePtr = pMeshTexture->pTexturesRef;
					if (pTexturePtr)
					{
...
/*
void WickedCall_DrawImguiNow( wiGraphics::CommandList cmd )
{
	void ImGui_RenderCall_Direct( wiGrpahics::CommandList cmd );
	ImGui_RenderCall_Direct( cmd );
}
*/
		...
		for (int a = 0; a < 13; a++) //TEXTURESLOT_COUNT = 13
		{
			if (!pScene->materials[i].textures[a].name.empty())
			{
				...
				if (image)
				{
					...
					void *pGPUResource = (void *)pScene->materials[i].textures[a].GetGPUResource();
					bool bAlreadyDisplayed = false;
					for(int b=0;b< already_registred.size();b++)
						if (already_registred[b] == pGPUResource) { bAlreadyDisplayed = true; break; }
					if (!bAlreadyDisplayed)
					{
						already_registred.push_back(pGPUResource);
						...
						float bperpixel = wiHelper::GetBitsPerPixel(Format) / 8.0f;
						int addmipmapssize;
						...
						sprintf(timestampMsg, "WList%d: (%ld,%ld) (%ld kb.+ mipm %ld kb.) filesize %ldkb mipmaps %d array %d format %s \"%s\" (*%p)", i, Width, Height, (int)((float)(Width*Height) * bperpixel) / 1024 * imgdesc.ArraySize, addmipmapssize, (long)filesize / 1024.0, imgdesc.MipLevels, imgdesc.ArraySize,wiHelper::GetFormatName(Format), pScene->materials[i].textures[a].name.c_str(), pGPUResource);
						timestampactivity(0, timestampMsg);
					}
				}
			}
