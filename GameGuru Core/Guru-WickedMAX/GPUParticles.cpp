...
DepthStencilState depthDescWriteEnabled;
DepthStencilState depthDescWriteDisabled;
DepthStencilState depthDescDisabled;

BlendState blendDescAlpha;
BlendState blendDescAdditive;
BlendState blendDescOpaque;
...
// called from WickedEngine wiRenderer::UpdateRenderData()
extern "C" void gpup_update_render_data( const CameraComponent& camera, CommandList cmd )
{
	GraphicsDevice* device = wiRenderer::GetDevice();

	// all vertex constant data must be changed here, outside the render pass, can't do it in gpup_draw()

	for ( size_t enr = 0; enr < gpup_maxeffects; ++enr )
	{
		if ( gpup_emitter[enr].effectLoaded == 0 || gpup_emitter[enr].effectVisible == 0 ) continue;

		gpup_emitter[enr].mainConstantData.Proj = camera.Projection;
		gpup_emitter[enr].mainConstantData.View = camera.View;

		XMMATRIX proj( (float*) &camera.Projection );
		XMMATRIX viewProj( (float*) &camera.View );
		viewProj *= proj;
		memcpy( gpup_emitter[enr].mainConstantData.ViewProj.m, viewProj.r, sizeof(float) * 16 );

		gpup_emitter[enr].mainConstantData.CameraPos.x = camera.Eye.x;
		gpup_emitter[enr].mainConstantData.CameraPos.y = camera.Eye.y;
		gpup_emitter[enr].mainConstantData.CameraPos.z = camera.Eye.z;

		float scale = 1.0f;
		gpup_emitter[enr].mainConstantData.World.m[0][0] = scale;
		gpup_emitter[enr].mainConstantData.World.m[0][1] = 0;
		gpup_emitter[enr].mainConstantData.World.m[0][2] = 0;
		gpup_emitter[enr].mainConstantData.World.m[0][3] = 0;
		gpup_emitter[enr].mainConstantData.World.m[1][0] = 0;
		gpup_emitter[enr].mainConstantData.World.m[1][1] = scale;
		gpup_emitter[enr].mainConstantData.World.m[1][2] = 0;
		gpup_emitter[enr].mainConstantData.World.m[1][3] = 0;
		gpup_emitter[enr].mainConstantData.World.m[2][0] = 0;
		gpup_emitter[enr].mainConstantData.World.m[2][1] = 0;
		gpup_emitter[enr].mainConstantData.World.m[2][2] = scale;
		gpup_emitter[enr].mainConstantData.World.m[2][3] = 0;
		gpup_emitter[enr].mainConstantData.World.m[3][3] = 1;

		gpup_emitter[enr].mainConstantData.agk_time = AGKTimer();

		device->UpdateBuffer( &mainConstants, &gpup_emitter[enr].mainConstantData, cmd, sizeof(sMainConstantData) );		
	}
}
...
extern "C" void gpup_draw( const CameraComponent& camera, CommandList cmd )
{
	...
	// can't update any buffers in this function as it is inside a render pass
	...
	for( size_t i = 0; i < gpup_maxeffects; ++i )
	{
		...
		switch( gpup_emitter[e].blendmode )
		{
			...
			case 2: // opaque
			{
				if (gpup_emitter[e].mainConstantData.opacity != 1.0f)
				{
					device->BindPipelineState(&psoAlpha, cmd); //If using opacity switch to alpha blend.
					device->BindBlendFactor(1.0f, 1.0f, 1.0f, 1.0f, cmd);
				}
				else
				{
					device->BindPipelineState(&psoOpaque, cmd); //Sprites
					device->BindBlendFactor(1.0f, 1.0f, 0.998f, 1.0f, cmd);
				}
			...
			case 4: // additive
			{
				...
				device->BindBlendFactor(1.0f, 1.0f, 0.999f, 1.0f, cmd);
			} break;
		...
		device->BindConstantBuffer( VS, &mainConstants, 2, cmd );
		device->BindConstantBuffer( PS, &mainConstants, 2, cmd );

		int numIndices = 0;
		const GPUBuffer* vbs[ 2 ];
		...
		vbs[ 1 ] = &gpup_emitter[e].mainInstanceBuffer;

		uint32_t stride[ 2 ] = { sizeof(GPUP_1024_Vertex), sizeof(InstancePos) }; 
		device->BindVertexBuffers( vbs, 0, 2, stride, 0, cmd );
		...
		uint32_t instanceCount = ii * ii;
	
		device->DrawIndexedInstanced( numIndices, instanceCount, 0, 0, 0, cmd );
...
void gpup_updatesettings( int enr )
{
	...
	gpup_emitter[enr].mainConstantData.clr.x = gpup_emitter[enr].r;
	gpup_emitter[enr].mainConstantData.clr.y = gpup_emitter[enr].g;
	gpup_emitter[enr].mainConstantData.clr.z = gpup_emitter[enr].b;
	gpup_emitter[enr].mainConstantData.opacity = 1.0f;
	...
	gpup_emitter[enr].mainConstantData.area.x = gpup_emitter[enr].area;
	gpup_emitter[enr].mainConstantData.area.y = gpup_emitter[enr].varea;
	gpup_emitter[enr].mainConstantData.area.z = 64;
	...
	gpup_emitter[enr].mainConstantData.pgrow.x = 0.001f+gpup_emitter[enr].size1*gpup_emitter[enr].sizer;
	gpup_emitter[enr].mainConstantData.pgrow.y = 0.001f+gpup_emitter[enr].size2*gpup_emitter[enr].sizer;
	gpup_emitter[enr].mainConstantData.pgrow.z = gpup_emitter[enr].particleSize;
	...
	gpup_emitter[enr].mainConstantData.particles.x = (float) particles;
	gpup_emitter[enr].mainConstantData.particles.y = (float) (particles*particles);
	gpup_emitter[enr].mainConstantData.particles.z = (float) (particles*particles-1);
	gpup_emitter[enr].mainConstantData.particles.w = (float) gpup_emitter[enr].facing;
	...
	gpup_emitter[enr].mainConstantData.image_count.x = (float) (gpup_emitter[enr].image_count+1);
	gpup_emitter[enr].mainConstantData.image_count.y = gpup_emitter[enr].image_distortion;
	gpup_emitter[enr].mainConstantData.image_count.z = gpup_emitter[enr].image_distortion_speed;
	...
	for( int i = 0; i < 8; i++ )
	{
		...
		{
			gpup_emitter[enr].mainConstantData.globalpos[bop].x = gpup_emitter[enr].globalx[i];
			gpup_emitter[enr].mainConstantData.globalpos[bop].y = gpup_emitter[enr].globaly[i];
			gpup_emitter[enr].mainConstantData.globalpos[bop].z = gpup_emitter[enr].globalz[i];
			gpup_emitter[enr].mainConstantData.globalpos[bop].w = (float) subs;

			bop++;
		}
	}
		
	gpup_emitter[enr].mainConstantData.globalsize.x = gpup_emitter[enr].globalSize;
	gpup_emitter[enr].mainConstantData.globalsize.y = gpup_emitter[enr].globalSize;
	gpup_emitter[enr].mainConstantData.globalsize.z = gpup_emitter[enr].globalSize;
	
	gpup_emitter[enr].mainConstantData.globalrot.x = 0;// gpup_emitter[enr].globalRotX; (See above for new use of globalrot)
	gpup_emitter[enr].mainConstantData.globalrot.y = 0;// gpup_emitter[enr].globalRotY;
	gpup_emitter[enr].mainConstantData.globalrot.z = 0;// gpup_emitter[enr].globalRotZ;
...
{	
	...
	gpup_emitter[enr].mainConstantData.globalsize.x = 1;
	gpup_emitter[enr].mainConstantData.globalsize.y = 1;
	gpup_emitter[enr].mainConstantData.globalsize.z = 1;
	
	gpup_emitter[enr].mainConstantData.globalrot.x = 0;
	gpup_emitter[enr].mainConstantData.globalrot.y = 0;
	gpup_emitter[enr].mainConstantData.globalrot.z = 0;
	
	gpup_emitter[enr].mainConstantData.particles.x = (float) selcol;
	gpup_emitter[enr].mainConstantData.particles.y = (float) (selcol*selcol);
	gpup_emitter[enr].mainConstantData.particles.z = (float) (selcol*selcol-1);
	gpup_emitter[enr].mainConstantData.particles.w = (float) gpup_emitter[enr].facing ;
	...
	int ii = gpup_emitter[enr].particles / 64;
	if ( gpup_emitter[enr].particles == 32 ) ii = 1;

	InstancePos* posData = new InstancePos[ ii * ii ];
	for( int i = 0; i < ii; i++ )
	{
		for( int j = 0; j < ii; j++ )
		{
			posData[ i*ii + j ].x = 64.0f * i;
			posData[ i*ii + j ].z = 64.0f * j;
		}
	}
	...
	bufferDesc.ByteWidth = sizeof(InstancePos) * ii*ii;
	bufferDesc.BindFlags = BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = 0;
	bufferDesc.MiscFlags = 0;
	wiRenderer::GetDevice()->CreateBuffer( &bufferDesc, &data, &gpup_emitter[enr].mainInstanceBuffer );
...
int gpup_loadEffectFile( const char* fl, float x, float y, float z, float s, float qual, float ps )
{
	...
	gpup_emitter[enr].mainConstantData.tile.x = (float) gpup_emitter[enr].ani_x;
	gpup_emitter[enr].mainConstantData.tile.y = (float) gpup_emitter[enr].ani_y;
	gpup_emitter[enr].mainConstantData.tile.z = (float) gpup_emitter[enr].ani_s;
	gpup_emitter[enr].mainConstantData.tile.w = (float) gpup_emitter[enr].smooth_blending ;
	
	gpup_emitter[enr].mainConstantData.tilex.x = (float) gpup_emitter[enr].ani_x;
	gpup_emitter[enr].mainConstantData.tilex.y = (float) gpup_emitter[enr].ani_y;
	gpup_emitter[enr].mainConstantData.tilex.z = (float) gpup_emitter[enr].ani_s;
	gpup_emitter[enr].mainConstantData.tilex.w = (float) gpup_emitter[enr].smooth_blending ;
	...
	gpup_emitter[enr].mainConstantData.image_count.x = (float) (gpup_emitter[enr].image_count+1);
	gpup_emitter[enr].mainConstantData.image_count.y = 0;
	gpup_emitter[enr].mainConstantData.image_count.z = 0;
...
void  gpup_doit( int enr, CommandList cmd )
{
	...
	{		
		...
		device->BindConstantBuffer( PS, &noiseConstants, 2, cmd );
		device->BindPipelineState( &psoNoise, cmd );
		device->BindResource( PS, &texNoiseOrig, 0, cmd );

		GPUBarrier barrier = GPUBarrier::Image( &gpup_emitter[enr].texNoise, IMAGE_LAYOUT_SHADER_RESOURCE, IMAGE_LAYOUT_RENDERTARGET );
		device->Barrier( &barrier, 1, cmd);

		GPUParticlesDrawQuad( &gpup_emitter[enr].renderPassNoise, cmd );

		barrier = GPUBarrier::Image( &gpup_emitter[enr].texNoise, IMAGE_LAYOUT_RENDERTARGET, IMAGE_LAYOUT_SHADER_RESOURCE );
		device->Barrier( &barrier, 1, cmd);
		...
		if ( gpup_emitter[enr].activeTimer > 0.0f )
		{
			...
			if ( gpup_emitter[enr].pivotMoved > 0 )
			{
				...
				gpup_emitter[enr].mainConstantData.globalpos[0].x = gpup_emitter[enr].globalx[0];
				gpup_emitter[enr].mainConstantData.globalpos[0].y = gpup_emitter[enr].globaly[0];
				gpup_emitter[enr].mainConstantData.globalpos[0].z = gpup_emitter[enr].globalz[0];
				gpup_emitter[enr].mainConstantData.globalpos[0].w = (float) gpup_getSubPositionCount(enr) ;
			}
			...
			device->BindConstantBuffer( PS, &speedConstants, 2, cmd );
			...
			GPUBarrier barrier = GPUBarrier::Image( &(gpup_emitter[enr].texSpeed[nextImage]), IMAGE_LAYOUT_SHADER_RESOURCE, IMAGE_LAYOUT_RENDERTARGET );
			device->Barrier( &barrier, 1, cmd);

			GPUParticlesDrawQuad( &gpup_emitter[enr].renderPassSpeed[nextImage], cmd );

			barrier = GPUBarrier::Image( &(gpup_emitter[enr].texSpeed[nextImage]), IMAGE_LAYOUT_RENDERTARGET, IMAGE_LAYOUT_SHADER_RESOURCE );
			device->Barrier( &barrier, 1, cmd);
			...
			device->BindConstantBuffer( PS, &posConstants, 2, cmd );
			device->BindPipelineState( &psoPos, cmd );
			device->BindResource( PS, &gpup_emitter[enr].texPos[currImage], 0, cmd );
			device->BindResource( PS, &gpup_emitter[enr].texSpeed[nextImage], 1, cmd );

			barrier = GPUBarrier::Image( &(gpup_emitter[enr].texPos[nextImage]), IMAGE_LAYOUT_SHADER_RESOURCE, IMAGE_LAYOUT_RENDERTARGET );
			device->Barrier( &barrier, 1, cmd);

			GPUParticlesDrawQuad( &gpup_emitter[enr].renderPassPos[nextImage], cmd );

			barrier = GPUBarrier::Image( &(gpup_emitter[enr].texPos[nextImage]), IMAGE_LAYOUT_RENDERTARGET, IMAGE_LAYOUT_SHADER_RESOURCE );
			device->Barrier( &barrier, 1, cmd);
		...
		gpup_emitter[enr].mainConstantData.rota.x = gpup_emitter[enr].rotation-0.5f;
		gpup_emitter[enr].mainConstantData.rota.y = gpup_emitter[enr].rotation_variance;
		gpup_emitter[enr].mainConstantData.rota.z = gpup_settings.sn;
...
int gpup_init()
{
	...
	depthDescWriteDisabled.DepthEnable = true;
	depthDescWriteDisabled.DepthWriteMask = DEPTH_WRITE_MASK_ZERO;
	depthDescWriteDisabled.DepthFunc = COMPARISON_GREATER_EQUAL;
	depthDescWriteDisabled.StencilEnable = false;

	depthDescWriteEnabled.DepthEnable = true;
	depthDescWriteEnabled.DepthWriteMask = DEPTH_WRITE_MASK_ALL;
	depthDescWriteEnabled.DepthFunc = COMPARISON_GREATER_EQUAL;
	depthDescWriteEnabled.StencilEnable = false;

	depthDescDisabled.DepthEnable = false;
	depthDescDisabled.DepthWriteMask = DEPTH_WRITE_MASK_ALL;
	depthDescDisabled.DepthFunc = COMPARISON_GREATER_EQUAL;
	depthDescDisabled.StencilEnable = false;
		
	blendDescAlpha.RenderTarget[0].BlendEnable = true;
	blendDescAlpha.RenderTarget[0].SrcBlend = BLEND_SRC_ALPHA;
	blendDescAlpha.RenderTarget[0].DestBlend = BLEND_INV_SRC_ALPHA;
	blendDescAlpha.RenderTarget[0].BlendOp = BLEND_OP_ADD;
	blendDescAlpha.RenderTarget[0].SrcBlendAlpha = BLEND_ONE;
	blendDescAlpha.RenderTarget[0].DestBlendAlpha = BLEND_INV_SRC_ALPHA;
	blendDescAlpha.RenderTarget[0].BlendOpAlpha = BLEND_OP_ADD;
	blendDescAlpha.RenderTarget[0].RenderTargetWriteMask = COLOR_WRITE_ENABLE_ALL;
	blendDescAlpha.IndependentBlendEnable = false;

	blendDescAdditive.RenderTarget[0].BlendEnable = true;
	blendDescAdditive.RenderTarget[0].SrcBlend = BLEND_SRC_ALPHA;
	blendDescAdditive.RenderTarget[0].DestBlend = BLEND_ONE;
	blendDescAdditive.RenderTarget[0].BlendOp = BLEND_OP_ADD;
	blendDescAdditive.RenderTarget[0].SrcBlendAlpha = BLEND_ZERO;
	blendDescAdditive.RenderTarget[0].DestBlendAlpha = BLEND_ONE;
	blendDescAdditive.RenderTarget[0].BlendOpAlpha = BLEND_OP_ADD;
	blendDescAdditive.RenderTarget[0].RenderTargetWriteMask = COLOR_WRITE_ENABLE_ALL;
	blendDescAdditive.IndependentBlendEnable = false;

	blendDescOpaque.RenderTarget[0].BlendEnable = false;
	blendDescOpaque.RenderTarget[0].SrcBlend = BLEND_SRC_ALPHA;
	blendDescOpaque.RenderTarget[0].DestBlend = BLEND_INV_SRC_ALPHA;
	blendDescOpaque.RenderTarget[0].BlendOp = BLEND_OP_ADD;
	blendDescOpaque.RenderTarget[0].SrcBlendAlpha = BLEND_ONE;
	blendDescOpaque.RenderTarget[0].DestBlendAlpha = BLEND_INV_SRC_ALPHA;
	blendDescOpaque.RenderTarget[0].BlendOpAlpha = BLEND_OP_ADD;
	blendDescOpaque.RenderTarget[0].RenderTargetWriteMask = COLOR_WRITE_ENABLE_ALL;
	blendDescOpaque.IndependentBlendEnable = false;
	...
	layoutDesc.elements =
	{
		...
		{ "OFFSET",   0, wiGraphics::FORMAT_R32G32_FLOAT,    1,  0, INPUT_PER_INSTANCE_DATA },
	};
	...
	desc.rs = &rasterDesc;
	desc.dss = &depthDescWriteDisabled;
	desc.bs = &blendDescAlpha;
	device->CreatePipelineState( &desc, &psoAlpha );

	// additive
	desc.bs = &blendDescAdditive;
	device->CreatePipelineState( &desc, &psoAdd );

	// opaque
	desc.dss = &depthDescWriteEnabled;
	desc.bs = &blendDescOpaque;
	...
	bd.Usage = USAGE_DEFAULT;
	bd.ByteWidth = sizeof(sMainConstantData);
	bd.BindFlags = BIND_CONSTANT_BUFFER;
	bd.CPUAccessFlags = 0;
	bd.MiscFlags = 0;
	wiRenderer::GetDevice()->CreateBuffer( &bd, nullptr, &mainConstants );
...
void gpup_setGlobalPosition( int ID, float x, float y, float z )
{
	...
	{
		gpup_emitter[ID].mainConstantData.globalpos[0].x = gpup_emitter[ID].globalx[0];
		gpup_emitter[ID].mainConstantData.globalpos[0].y = gpup_emitter[ID].globaly[0];
		gpup_emitter[ID].mainConstantData.globalpos[0].z = gpup_emitter[ID].globalz[0];
		gpup_emitter[ID].mainConstantData.globalpos[0].w = (float) gpup_getSubPositionCount(ID) ;
	}
}
...
void gpup_setGlobalScale( int ID, float s )
{
	...
	if ( gpup_emitter[ID].effectLoaded == 1 )
	{
		gpup_emitter[ID].mainConstantData.globalsize.x = gpup_emitter[ID].globalSize;
		gpup_emitter[ID].mainConstantData.globalsize.y = gpup_emitter[ID].globalSize;
		gpup_emitter[ID].mainConstantData.globalsize.z = gpup_emitter[ID].globalSize;
	}
}
...
void gpup_setParticleScale( int ID, float s )
{
	...
	if ( gpup_emitter[ID].effectLoaded == 1 ) 
	{
		gpup_emitter[ID].mainConstantData.pgrow.x = 0.001f+gpup_emitter[ID].size1*gpup_emitter[ID].sizer;
		gpup_emitter[ID].mainConstantData.pgrow.y = 0.001f+gpup_emitter[ID].size2*gpup_emitter[ID].sizer;
		gpup_emitter[ID].mainConstantData.pgrow.z = gpup_emitter[ID].particleSize;
	}
}
...
void gpup_setGlobalRotation( int ID, float x, float y, float z )
{
	...
	if ( gpup_emitter[ID].effectLoaded == 1 )
	{
		gpup_emitter[ID].mainConstantData.globalrot.x = 0;// gpup_emitter[ID].globalRotX; (See new use of globalrot)
		gpup_emitter[ID].mainConstantData.globalrot.y = 0;// gpup_emitter[ID].globalRotY;
		gpup_emitter[ID].mainConstantData.globalrot.z = 0;// gpup_emitter[ID].globalRotZ;
	}
}
...
void gpup_setBlendmode( int ID, int usel )
{
	...
	{
		if ( usel == 0 ) usel = 1;
		gpup_emitter[ID].mainConstantData.moder = (float) usel;
	}
}
...
void gpup_setEffectColor( int ID, int r, int g, int b )
{
	...
	if ( gpup_emitter[ID].effectLoaded == 1 ) 
	{
		gpup_emitter[ID].mainConstantData.clr.x = gpup_emitter[ID].r;
		gpup_emitter[ID].mainConstantData.clr.y = gpup_emitter[ID].g;
		gpup_emitter[ID].mainConstantData.clr.z = gpup_emitter[ID].b;
	}
}
...
void gpup_setEffectOpacity(int ID, float a)
{
	...
	if (gpup_emitter[ID].effectLoaded == 1)
	{
		gpup_emitter[ID].mainConstantData.opacity = gpup_emitter[ID].opacity;
	}
}
...
void gpup_setSmoothBlending( int ID, int act )
{
	...
	if ( gpup_emitter[ID].effectLoaded == 1  )
	{
		...
		gpup_emitter[ID].mainConstantData.tile.x = (float) gpup_emitter[ID].ani_x;
		gpup_emitter[ID].mainConstantData.tile.y = (float) gpup_emitter[ID].ani_y;
		gpup_emitter[ID].mainConstantData.tile.z = (float) gpup_emitter[ID].ani_s;
		gpup_emitter[ID].mainConstantData.tile.w = (float) act;
		
		gpup_emitter[ID].mainConstantData.tilex.x = (float) gpup_emitter[ID].ani_x;
		gpup_emitter[ID].mainConstantData.tilex.y = (float) gpup_emitter[ID].ani_y;
		gpup_emitter[ID].mainConstantData.tilex.z = (float) gpup_emitter[ID].ani_s;
		gpup_emitter[ID].mainConstantData.tilex.w = (float) act;
	}
}
...
int gpup_deleteEffect( int ID )
{
	...
	// delete gpup_emitter[ID].mainConstantData
	// delete gpup_emitter[ID].mainConstantData
