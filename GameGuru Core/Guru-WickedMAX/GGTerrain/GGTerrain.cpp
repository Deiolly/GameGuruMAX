...
// rendering variables
RasterizerState rastStateCullBackCW;
RasterizerState rastStateCullBackCCW;
RasterizerState rastStateWireframeCullBackCW;
RasterizerState rastStateShadowCullBackCW;
RasterizerState rastStateDoubleSidedCCW;

DepthStencilState depthStateWriteEnabled;
DepthStencilState depthStateWriteDisabled;
...
public:
	...
	void UpdateLODTextures( GGTerrainLODSet* pLODs, int fullUpdate, CommandList cmd )
	{
		GraphicsDevice* device = wiRenderer::GetDevice();
		...
		for( uint32_t lod = 0; lod < pLODs->GetNumLevels(); lod++ )
		{
			...
			for( uint32_t y = 0; y < 8; y++ )
			{
				for( uint32_t x = 0; x < 8; x++ )
				{
					...
					device->UpdateTexture( &texLODHeightMapArray, 0, lod, &dstBox, pLevel->chunkGrid[ index ]->pHeightMap, stride, cmd ); 
					device->UpdateTexture( &texLODNormalMapArray, 0, lod, &dstBox, pLevel->chunkGrid[ index ]->pNormalMap, stride, cmd ); 
				}
			}
		}
	}
	...
	void UpdateChunks( float playerX, float playerZ, CommandList cmd )
	{
		...
		if ( pNewLODs->IsGenerating() )
		{
			...
			if ( pCurrLODs->IsValid() ) 
			{
				...
				UpdateLODTextures( pNewLODs, 0, cmd );
			}
			else 
			{
				iFlags |= GGTERRAIN_FLAG_REGENERATE_PAGES;
				UpdateLODTextures( pNewLODs, 1, cmd );
		...
		if ( pCurrLODs->IsGenerating() )
		{
			...
			else 
			{
				...
				iFlags |= GGTERRAIN_FLAG_REGENERATE_PAGES;
				UpdateLODTextures( pCurrLODs, 1, cmd );
				WickedCall_UpdateProbes();
				ggterrain_extra_params.bTerrainChanged = true;
				ggterrain_extra_params.iUpdateTrees = 20;
				ggterrain_extra_params.iUpdateGrass = 20;
...
void GGTerrain_CreateFractalTexture( Texture* tex, uint32_t size, CommandList cmd ) 
{ 
	GraphicsDevice* device = wiRenderer::GetDevice();
	...
	*/
	...
	device->UpdateTexture( tex, 0, 0, 0, imageData, rowStride, cmd );
	delete [] imageData;

	// generate all mip levels from mip 0
	device->GenerateMipmaps( tex, cmd );
}

void GGTerrain_LoadTexturePNG( const char* filename, Texture* tex, CommandList cmd ) 
{ 
	GraphicsDevice* device = wiRenderer::GetDevice();
	...
	device->UpdateTexture( tex, 0, 0, 0, imageData, rowStride, cmd );  //for when new Wicked Terrain Shader is created
...
void GGTerrain_LoadTextureDDSIntoSlice( const char* filename, Texture* tex, uint32_t arraySlice, CommandList cmd ) 
{ 
	GraphicsDevice* device = wiRenderer::GetDevice();
...
void GGTerrain_CreateComputeTexture( int width, int height, FORMAT format, Texture* tex ) 
{
	...
	texDesc.Width = width;
	texDesc.Height = height;
	texDesc.layout = IMAGE_LAYOUT_UNORDERED_ACCESS;
...
int GGTerrain_Init( wiGraphics::CommandList cmd )
{
	...
	*/
	...
	GGTerrain_CreateFractalTexture( &texMask, 1024, cmd );
	...
	for( uint32_t i = 0; i < GGTERRAIN_MAX_SOURCE_TEXTURES; i++ )
	{
		...
		//sprintf_s( szDstRoot, MAX_PATH, "Files/terraintextures/%s/%s", terrainTextureNames[ i ], terrainTextureNames[ i ] + 5 );
		...
		GGTerrain_LoadTextureDDSIntoSlice( szDstColorPath, &texColorArray, i, cmd );
		GGTerrain_LoadTextureDDSIntoSlice( szDstNormalPath, &texNormalsArray, i, cmd );

#ifdef GGTERRAIN_USE_SURFACE_TEXTURE
		char szDstSurfacePath[ MAX_PATH ];
		sprintf_s( szDstSurfacePath, MAX_PATH, "%s/Surface.dds", szDstRoot );
		GGTerrain_LoadTextureDDSIntoSlice( szDstSurfacePath, &texSurfaceArray, i, cmd );
#else
		char szDstRoughnessMetalnessPath[ MAX_PATH ];
		char szDstAOPath[ MAX_PATH ];
		sprintf_s( szDstRoughnessMetalnessPath, MAX_PATH, "%s/RoughnessMetalness.dds", szDstRoot );
		sprintf_s( szDstAOPath, MAX_PATH, "%s/AO.dds", szDstRoot );
		GGTerrain_LoadTextureDDSIntoSlice( szDstRoughnessMetalnessPath, &texRoughnessMetalnessArray, i );
		GGTerrain_LoadTextureDDSIntoSlice( szDstAOPath, &texAOArray, i );
#endif
	}
	...
	*/
	device->UpdateTexture( &texMaterialMap, 0, 0, NULL, pMaterialMap, GGTERRAIN_MATERIALMAP_SIZE, cmd );
	...
	//GGTerrain_AddFlatCircle( 0, 0, 1000 );
	//GGTerrain_AddFlatRect( 0, 0, 1000, 2000, 0 );
	...
	{
		device->UpdateTexture( &texPageTableFinal, mipLevel, 0, 0, pageTableData, stride, cmd );
		stride /= 2;
	}
...
/*
	...
	*/
	//device->UpdateTexture( &texPagesColorAndMetal, 0, 0, 0, phyData, physTexSizeX*sizeof(uint32_t), cmd );
	...
	//device->UpdateTexture( &texPagesColorAndMetal, 1, 0, 0, phyData, physTexSizeX*sizeof(uint32_t)/2, cmd );
	//delete [] phyData;
	...
	// raster state
	rastStateCullBackCW = {};
	rastStateCullBackCW.FillMode = FILL_SOLID;
	rastStateCullBackCW.CullMode = CULL_BACK;
	rastStateCullBackCW.FrontCounterClockwise = false;
	rastStateCullBackCW.DepthBias = 0;
	rastStateCullBackCW.DepthBiasClamp = 0;
	rastStateCullBackCW.SlopeScaledDepthBias = 0;
	rastStateCullBackCW.DepthClipEnable = true;
	rastStateCullBackCW.MultisampleEnable = false;
	rastStateCullBackCW.AntialiasedLineEnable = false;

	rastStateCullBackCCW = {};
	rastStateCullBackCCW.FillMode = FILL_SOLID;
	rastStateCullBackCCW.CullMode = CULL_BACK;
	rastStateCullBackCCW.FrontCounterClockwise = true;
	rastStateCullBackCCW.DepthBias = 0;
	rastStateCullBackCCW.DepthBiasClamp = 0;
	rastStateCullBackCCW.SlopeScaledDepthBias = 0;
	rastStateCullBackCCW.DepthClipEnable = true;
	rastStateCullBackCCW.MultisampleEnable = false;
	rastStateCullBackCCW.AntialiasedLineEnable = false;

	rastStateWireframeCullBackCW = {};
	rastStateWireframeCullBackCW.FillMode = FILL_WIREFRAME;
	rastStateWireframeCullBackCW.CullMode = CULL_BACK;
	rastStateWireframeCullBackCW.FrontCounterClockwise = false;
	rastStateWireframeCullBackCW.DepthBias = 0;
	rastStateWireframeCullBackCW.DepthBiasClamp = 0;
	rastStateWireframeCullBackCW.SlopeScaledDepthBias = 0;
	rastStateWireframeCullBackCW.DepthClipEnable = true;
	rastStateWireframeCullBackCW.MultisampleEnable = false;
	rastStateWireframeCullBackCW.AntialiasedLineEnable = false;
	
	rastStateShadowCullBackCW = {};
	rastStateShadowCullBackCW.FillMode = FILL_SOLID;
	rastStateShadowCullBackCW.CullMode = CULL_BACK;
	rastStateShadowCullBackCW.FrontCounterClockwise = false;
	rastStateShadowCullBackCW.DepthBias = -1;
	rastStateShadowCullBackCW.DepthBiasClamp = 0;
	rastStateShadowCullBackCW.SlopeScaledDepthBias = -4.0f;
	rastStateShadowCullBackCW.DepthClipEnable = false;
	rastStateShadowCullBackCW.MultisampleEnable = false;
	rastStateShadowCullBackCW.AntialiasedLineEnable = false;

	// for transparent items
	rastStateDoubleSidedCCW = {};
	rastStateDoubleSidedCCW.FillMode = FILL_SOLID;
	rastStateDoubleSidedCCW.CullMode = CULL_NONE;
	rastStateDoubleSidedCCW.FrontCounterClockwise = true;
	rastStateDoubleSidedCCW.DepthBias = 0;
	rastStateDoubleSidedCCW.DepthBiasClamp = 0;
	rastStateDoubleSidedCCW.SlopeScaledDepthBias = 0;
	rastStateDoubleSidedCCW.DepthClipEnable = true;
	rastStateDoubleSidedCCW.MultisampleEnable = false;
	rastStateDoubleSidedCCW.AntialiasedLineEnable = false;
	
	// depth stencil state
	depthStateWriteEnabled = {};
	depthStateWriteEnabled.DepthEnable = true;
	depthStateWriteEnabled.DepthFunc = COMPARISON_GREATER_EQUAL;
	depthStateWriteEnabled.StencilEnable = false;
	depthStateWriteEnabled.DepthWriteMask = DEPTH_WRITE_MASK_ALL;

	depthStateWriteDisabled = {};
	depthStateWriteDisabled.DepthEnable = true;
	depthStateWriteDisabled.DepthFunc = COMPARISON_GREATER_EQUAL;
	depthStateWriteDisabled.StencilEnable = false;
	depthStateWriteDisabled.DepthWriteMask = DEPTH_WRITE_MASK_ZERO;

	depthStateDisabled = {};
	depthStateDisabled.DepthEnable = false;
	depthStateDisabled.DepthFunc = COMPARISON_GREATER_EQUAL;
	depthStateDisabled.StencilEnable = false;
	depthStateDisabled.DepthWriteMask = DEPTH_WRITE_MASK_ZERO;
	...
	blendStateAlpha.RenderTarget[0].SrcBlend = BLEND_SRC_ALPHA;
	blendStateAlpha.RenderTarget[0].DestBlend = BLEND_INV_SRC_ALPHA;
	blendStateAlpha.RenderTarget[0].BlendOp = BLEND_OP_ADD;
	blendStateAlpha.RenderTarget[0].SrcBlendAlpha = BLEND_ONE;
	blendStateAlpha.RenderTarget[0].DestBlendAlpha = BLEND_ZERO;
	blendStateAlpha.RenderTarget[0].BlendOpAlpha = BLEND_OP_ADD;
	blendStateAlpha.RenderTarget[0].RenderTargetWriteMask = COLOR_WRITE_ENABLE_ALL;
	...
	desc.rs = &rastStateCullBackCW;
	desc.dss = &depthStateWriteDisabled;
	desc.bs = &blendStateOpaque;
	device->CreatePipelineState( &desc, &psoMain );

	desc.rs = &rastStateWireframeCullBackCW;
	desc.dss = &depthStateWriteEnabled;
	desc.bs = &blendStateOpaque;
	...
	desc.ps = nullptr;
	desc.rs = &rastStateShadowCullBackCW;
	desc.dss = &depthStateWriteEnabled;
	desc.bs = &blendStateOpaque;
	...
	bd.CPUAccessFlags = 0;
	bd.MiscFlags = 0;
	device->CreateBuffer( &bd, &data, &instanceBuffer );
	...
	desc.rs = &rastStateDoubleSidedCCW;
	desc.dss = &depthStateWriteDisabled;
	desc.bs = &blendStateAlpha;
	desc.pt = TRIANGLELIST;
	desc.il = &boxLayout;
...
void GGTerrain_DrawPages( CommandList cmd )
{
	...
	GPUBarrier barriers[] = {
		GPUBarrier::Image( &texPagesColorAndMetal, IMAGE_LAYOUT_SHADER_RESOURCE, IMAGE_LAYOUT_RENDERTARGET ),
		GPUBarrier::Image( &texPagesNormalsRoughnessAO, IMAGE_LAYOUT_SHADER_RESOURCE, IMAGE_LAYOUT_RENDERTARGET),
	};
	device->Barrier( barriers, 2, cmd);
	...
	GPUBarrier barriers2[] = {
		GPUBarrier::Image( &texPagesColorAndMetal, IMAGE_LAYOUT_RENDERTARGET, IMAGE_LAYOUT_SHADER_RESOURCE ),
		GPUBarrier::Image( &texPagesNormalsRoughnessAO, IMAGE_LAYOUT_RENDERTARGET, IMAGE_LAYOUT_SHADER_RESOURCE ),
	};
	device->Barrier( barriers2, 2, cmd);
...
int GGTerrain_GeneratePage( PageEntry* pPage, CommandList cmd )
{
	...
	if ( ggterrain.IsHighDetail( detailLevel ) )
	{
		...
		device->UpdateTexture( &texPageTableArray, 0, detailLevel, &dstBox, &pixel, 2, cmd ); 
	}
	else
	{
		uint32_t mipLevel = ggterrain.ConvertToMipLevel( detailLevel );
		pageTableData[ detailLevel ][ virtOffsetY * (pagesX >> mipLevel) + virtOffsetX ] = pixel;
		device->UpdateTexture( &texPageTableFinal, mipLevel, 0, &dstBox, &pixel, 2, cmd ); 
	}
...
void GGTerrain_CheckPageShift( CommandList cmd )
{
	GraphicsDevice* device = wiRenderer::GetDevice();
	...
	if ( ggterrain.ShouldRegeneratePages() )
	{
		...
		{
			device->UpdateTexture( &texPageTableArray, 0, level, 0, pageTableData, stride, cmd );
		}
		...
		for( int y = 0; y < 4; y++ )
		{
			for( int x = 0; x < 4; x++ )
			{
				...
				GGTerrain_GeneratePage( pPage, cmd );
			}
		}
	}
	else if ( ggterrain.ShouldShiftPages() )
	{
		...
		for( uint32_t level = 0; level < numLODLevels-1; level++ )
		{
			...
			uint32_t stride = pagesX * sizeof(uint16_t);
			device->UpdateTexture( &texPageTableArray, 0, level, 0, &pageTableData[ level ], stride, cmd );
		}
		...
		if ( shiftX != 0 || shiftY != 0 ) 
		{
			...
			for( uint32_t level = 0; level < GGTERRAIN_MAX_PAGE_TABLE_MIP; level++ )
			{
				...
				device->UpdateTexture( &texPageTableFinal, level, 0, 0, &pageTableData[ pageLevel ], stride, cmd );
			}
			...
			for( int y = 0; y < 4; y++ )
			{
				for( int x = 0; x < 4; x++ )
				{
					...
					uint32_t identifier = ((detailLevel + 1) << 16) | (y << 8) | x;
					pPage->Setup( identifier );
					int result = GGTerrain_GeneratePage( pPage, cmd );
					assert( result );
				}
			}
		}
...
void GGTerrain_CheckReadBack( CommandList cmd )
{
	GraphicsDevice* device = wiRenderer::GetDevice();
	...
	if ( readBackValid )
	{
		...
		for( uint32_t i = 0; i < numNewPages; i++ )
		{
			...
			int result = GGTerrain_GeneratePage( pPageEntry, cmd );
			assert( result );
		}
...
void GGTerrain_Update_Painting( float pickX, float pickY, float pickZ, CommandList cmd )
{
	...
	{		
		...
		if ( endX >= 1 && endY >= 1 && startX < GGTERRAIN_MATERIALMAP_SIZE-2 && startY < GGTERRAIN_MATERIALMAP_SIZE-2 )
		{
			...
			wiRenderer::GetDevice()->UpdateTexture( &texMaterialMap, 0, 0, NULL, pMaterialMap, GGTERRAIN_MATERIALMAP_SIZE, cmd );
			ggterrain_internal_params.update_material_texture = 0;
...
//	if ( !ggterrain_update_enabled ) return;
	...
	if ( ggterrain_update_enabled ) 
	{
		...
		ggterrain.UpdateChunks( playerX, playerZ, cmd );
	...
	if (pickHit && bRenderTargetFocus && !bImGuiGotFocus)
	{
		...
		switch( ggterrain_extra_params.edit_mode )
		{
			...
			{
				GGTerrain_Update_Painting( pickX, pickY, pickZ, cmd );
			} break;
	...
	if ( ggterrain_internal_params.update_material_texture )
	{
		wiRenderer::GetDevice()->UpdateTexture(&texMaterialMap, 0, 0, NULL, pMaterialMap, GGTERRAIN_MATERIALMAP_SIZE, cmd );
		ggterrain_internal_params.update_material_texture = 0;
	}
	...
	if ( !pCurrLODs->IsGenerating() )
	{
		GGTerrain_CheckPageShift( cmd );
		GGTerrain_CheckReadBack( cmd ); // CPU only side of the read back, GPU side is done in prepass render
	}
...
extern "C" void GGTerrain_VirtualTexReadBack( Texture texReadBack, uint32_t sampleCount, wiGraphics::CommandList cmd )
{
	...
	GPUBarrier barrier = GPUBarrier::Image( &texReadBackCompute, IMAGE_LAYOUT_UNORDERED_ACCESS, IMAGE_LAYOUT_COPY_SRC, 0, 0 );
	device->Barrier( &barrier, 1, cmd);

	device->CopyResource(&texReadBackStaging[currReadBackTex], &texReadBackCompute, cmd);

	barrier = GPUBarrier::Image( &texReadBackCompute, IMAGE_LAYOUT_COPY_SRC, IMAGE_LAYOUT_UNORDERED_ACCESS, 0, 0 );
	device->Barrier( &barrier, 1, cmd);
