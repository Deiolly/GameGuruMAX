...
RasterizerState rastStateDoubleSided;
RasterizerState rastStateCullBack;
RasterizerState rastStateShadowDoubleSided;
RasterizerState rastStateShadowCullBack;

DepthStencilState depthStateWriteDisable;
DepthStencilState depthStateWriteEnable;

BlendState blendStateOpaqueNoCoverage;
...
void GGTrees_LoadTextureDDSIntoSlice( const char* filename, Texture* tex, uint32_t arraySlice, CommandList cmd ) 
{ 
	GraphicsDevice* device = wiRenderer::GetDevice();
...
void GGTrees_Init( wiGraphics::CommandList cmd )
{
	GraphicsDevice* device = wiRenderer::GetDevice();
	...
	for( int i = 0; i < numTreeTypes; i++ )
	{
		...
		GGTrees_LoadTextureDDSIntoSlice( path, &texTree, i, cmd );

		strcpy_s( path, "Files/treebank/billboards/" );
		strcat_s( path, g_GGTrees[ i ].billboardNormalFilename );
		GGTrees_LoadTextureDDSIntoSlice( path, &texTreeNormal, i, cmd );

		strcpy_s( path, "Files/treebank/textures/" );
		strcat_s( path, g_GGTrees[ i ].trunk->textureName );
		GGTrees_LoadTextureDDSIntoSlice( path, &texTreeHigh, i, cmd );

		if ( g_GGTrees[ i ].branches )
		{
			strcpy_s( path, "Files/treebank/textures/" );
			strcat_s( path, g_GGTrees[ i ].branches->textureName );
			GGTrees_LoadTextureDDSIntoSlice( path, &texBranchesHigh, i, cmd );
		}
	...
	// raster state
	rastStateDoubleSided = {};
	rastStateDoubleSided.FillMode = FILL_SOLID;
	rastStateDoubleSided.CullMode = CULL_NONE;
	rastStateDoubleSided.FrontCounterClockwise = true;
	rastStateDoubleSided.DepthBias = 0;
	rastStateDoubleSided.DepthBiasClamp = 0;
	rastStateDoubleSided.SlopeScaledDepthBias = 0;
	rastStateDoubleSided.DepthClipEnable = true;
	rastStateDoubleSided.MultisampleEnable = false;
	rastStateDoubleSided.AntialiasedLineEnable = false;

	rastStateCullBack = {};
	rastStateCullBack.FillMode = FILL_SOLID;
	rastStateCullBack.CullMode = CULL_BACK;
	rastStateCullBack.FrontCounterClockwise = true;
	rastStateCullBack.DepthBias = 0;
	rastStateCullBack.DepthBiasClamp = 0;
	rastStateCullBack.SlopeScaledDepthBias = 0;
	rastStateCullBack.DepthClipEnable = true;
	rastStateCullBack.MultisampleEnable = false;
	rastStateCullBack.AntialiasedLineEnable = false;

	rastStateShadowDoubleSided = {};
	rastStateShadowDoubleSided.FillMode = FILL_SOLID;
	rastStateShadowDoubleSided.CullMode = CULL_NONE;
	rastStateShadowDoubleSided.FrontCounterClockwise = true;
	rastStateShadowDoubleSided.DepthBias = -1;
	rastStateShadowDoubleSided.DepthBiasClamp = 0;
	rastStateShadowDoubleSided.SlopeScaledDepthBias = -4.0f;
	rastStateShadowDoubleSided.DepthClipEnable = false;
	rastStateShadowDoubleSided.MultisampleEnable = false;
	rastStateShadowDoubleSided.AntialiasedLineEnable = false;
	
	rastStateShadowCullBack = {};
	rastStateShadowCullBack.FillMode = FILL_SOLID;
	rastStateShadowCullBack.CullMode = CULL_BACK;
	rastStateShadowCullBack.FrontCounterClockwise = true;
	rastStateShadowCullBack.DepthBias = -1;
	rastStateShadowCullBack.DepthBiasClamp = 0;
	rastStateShadowCullBack.SlopeScaledDepthBias = -4.0f;
	rastStateShadowCullBack.DepthClipEnable = false;
	rastStateShadowCullBack.MultisampleEnable = false;
	rastStateShadowCullBack.AntialiasedLineEnable = false;
	
	// depth stencil state
	depthStateWriteEnable = {};
	depthStateWriteEnable.DepthEnable = true;
	depthStateWriteEnable.DepthFunc = COMPARISON_GREATER_EQUAL;
	depthStateWriteEnable.StencilEnable = false;
	depthStateWriteEnable.DepthWriteMask = DEPTH_WRITE_MASK_ALL;

	depthStateWriteDisable = {};
	depthStateWriteDisable.DepthEnable = true;
	depthStateWriteDisable.DepthFunc = COMPARISON_GREATER_EQUAL;
	depthStateWriteDisable.StencilEnable = false;
	depthStateWriteDisable.DepthWriteMask = DEPTH_WRITE_MASK_ZERO;
	...
	blendStateOpaqueNoCoverage = {};
	blendStateOpaqueNoCoverage.RenderTarget[0].BlendEnable = false;
	blendStateOpaqueNoCoverage.RenderTarget[0].SrcBlend = BLEND_ONE;
	blendStateOpaqueNoCoverage.RenderTarget[0].DestBlend = BLEND_ZERO;
	blendStateOpaqueNoCoverage.RenderTarget[0].BlendOp = BLEND_OP_ADD;
	blendStateOpaqueNoCoverage.RenderTarget[0].SrcBlendAlpha = BLEND_ONE;
	blendStateOpaqueNoCoverage.RenderTarget[0].DestBlendAlpha = BLEND_ZERO;
	blendStateOpaqueNoCoverage.RenderTarget[0].BlendOpAlpha = BLEND_OP_ADD;
	blendStateOpaqueNoCoverage.RenderTarget[0].RenderTargetWriteMask = COLOR_WRITE_ENABLE_ALL;
	blendStateOpaqueNoCoverage.IndependentBlendEnable = false;
	blendStateOpaqueNoCoverage.AlphaToCoverageEnable = false;
	...
	desc.rs = &rastStateDoubleSided;
	desc.dss = &depthStateWriteDisable;
	desc.bs = &blendStateOpaque;
	device->CreatePipelineState( &desc, &psoTrees );

	desc.vs = &shaderTreesHighVS;
	desc.ps = &shaderTreesHighPS;
	desc.rs = &rastStateCullBack;
	desc.dss = &depthStateWriteDisable;
	desc.bs = &blendStateOpaqueNoCoverage;
	...
	desc.rs = &rastStateCullBack;
	desc.dss = &depthStateWriteEnable;
	desc.bs = &blendStateOpaqueNoCoverage;
	desc.il = &inputLayoutHigh;
	...
	desc.rs = &rastStateShadowDoubleSided;
	desc.dss = &depthStateWriteEnable;
	desc.bs = &blendStateOpaqueNoCoverage;
	desc.il = &inputLayout;
	...
	desc.il = &inputLayoutHigh;
	desc.rs = &rastStateShadowCullBack;
	desc.dss = &depthStateWriteEnable;
	desc.bs = &blendStateOpaqueNoCoverage;
