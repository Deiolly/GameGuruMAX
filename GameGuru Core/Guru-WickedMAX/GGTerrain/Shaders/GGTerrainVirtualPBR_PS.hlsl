...
inline void TiledLighting(inout Surface surface, inout Lighting lighting, float dist, out float3 envmapAmbient)
{
	...
	envmapAmbient = float3( 0,0,0 );

#ifndef DISABLE_ENVMAPS
	// Apply environment maps:
	float4 envmapAccumulation = float4(0,0,0,0);
			...
			while (bucket_bits != 0)
			{
				...
				if (entity_index >= first_item && entity_index <= last_item && envmapAccumulation.a < 1)
				{
					...
					if (is_saturated(uvw))
					{
						...
						float4 envmapColor = EnvironmentReflection_Local(surface, probe, probeProjection, clipSpacePos);
...
GBuffer main( PixelIn IN )
{
	...
	/*
	output.g0 = mipColors[detailLevel];
