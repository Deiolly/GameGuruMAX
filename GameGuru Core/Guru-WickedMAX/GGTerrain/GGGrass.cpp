...
RasterizerState rastStateShadow;
...
void GGGrass_LoadTextureDDSIntoSlice( const char* filename, Texture* tex, uint32_t arraySlice, CommandList cmd ) 
{ 
	GraphicsDevice* device = wiRenderer::GetDevice();
...
void GGGrass_Init( wiGraphics::CommandList cmd )
{
	GrassTimerInit();
	...
	for( uint32_t i = 0; i < GGGRASS_NUM_TYPES; i++ )
	{
		...
		GGGrass_LoadTextureDDSIntoSlice( grassFilename, &texGrass, i, cmd );
	}
	...
	rastStateShadow = {};
	rastStateShadow.FillMode = FILL_SOLID;
	rastStateShadow.CullMode = CULL_NONE;
	rastStateShadow.FrontCounterClockwise = true;
	rastStateShadow.DepthBias = -1;
	rastStateShadow.DepthBiasClamp = 0;
	rastStateShadow.SlopeScaledDepthBias = -4.0f;
	rastStateShadow.DepthClipEnable = false;
	rastStateShadow.MultisampleEnable = false;
	rastStateShadow.AntialiasedLineEnable = false;
...
int GGGrass_SetData( uint32_t size, uint8_t* data, sUndoSysEventGrass* pEvent)
{
	...
	{ 
		...
		//wiRenderer::GetDevice()->UpdateTexture(&texGrassMap, 0, 0, NULL, pGrassMap, GGGRASS_MAP_SIZE, cmd);
